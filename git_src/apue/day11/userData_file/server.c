...
// 类型 services_t 将客户端的信息封装（请求以及地址）
typedef struct {
	c2s_req_t 			cli_msg;		//客户端的服务请求
	struct sockaddr_in 	cli_addr;		//客户端的网络地址
}services_t;
...
{
	services_t *pmsg = (services_t*)arg;
	pfunc[pmsg->cli_msg.req_type](pmsg); 
...
int main()
{
	...
	while (1) {
		...
		//创建子线程去处理服务请求
		pthread_create(&tid, NULL, thr_func, (void*)msg);
	}
